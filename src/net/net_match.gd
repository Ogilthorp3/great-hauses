...
func _rpc_hello(their_protocol: int, house_id: String) -> void:
	...
	if their_protocol != NetProtocol.PROTOCOL_VERSION:
		...
		_rpc_refused.rpc_id(multiplayer.get_remote_sender_id(), why)
		_fail(why)
		return
	...
	_rpc_match_start.rpc_id(multiplayer.get_remote_sender_id(), {
...
func _rpc_request_move(req_seq: int, from_idx: int, to_idx: int, promo: String) -> void:
	if not is_host:
		return
	_handle_request(multiplayer.get_remote_sender_id(), req_seq, from_idx, to_idx, promo)
