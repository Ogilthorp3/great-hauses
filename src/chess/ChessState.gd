...
# Creates a ChessMove object from these squares. Does not check if the move is legal
func construct_move(from_square: int, to_square: int, promotion := "q") -> MoveScript:
	var move := MoveScript.new()
	move.from_square = from_square
	move.to_square = to_square
	move.captured_piece = pieces[to_square]
	move.piece = pieces[from_square]
	...
	if pieces[from_square].to_lower() == "k":
		...
		# Presentation metadata: castling rook path
		if to_square == from_square + 2:
			move.is_castling = true
			move.castle_kingside = true
			move.rook_from = from_square + 3
			move.rook_to = from_square + 1
		elif to_square == from_square - 2:
			move.is_castling = true
			move.castle_kingside = false
			move.rook_from = from_square - 4
			move.rook_to = from_square - 1
	...
	# Presentation metadata: where the captured piece actually sits
	if move.en_passant:
		move.captured_square = to_square + (-8 if turn else 8)
	elif move.captured_piece != null:
		move.captured_square = to_square

	return move


# Plays the given move on the board and updates the internal state accordingly
func play_move(move: MoveScript) -> void:
	if move.promotion:
...
func undo() -> void:
	...
	var move := move_stack.pop_back() as MoveScript
...
func legal_moves(notate_san := false) -> Array:
	return generate_legal_moves(notate_san)


func apply_move(move: MoveScript) -> void:
	play_move(move)


# Finds the legal move matching a UCI string like "e2e4" or "e7e8q". Returns null if not legal
func move_from_uci(uci: String) -> MoveScript:
	for move in generate_legal_moves(false):
		if move.to_uci() == uci:
			return move
	return null
...
# Alias kept for parity with the source engine's API
func get_data() -> int:
	return get_result()
...
func is_nfold_repetition(n: int) -> bool:
	...
	while board_reference.move_stack.size() > 0:
		...
		var last_move := board_reference.move_stack[-1] as MoveScript
