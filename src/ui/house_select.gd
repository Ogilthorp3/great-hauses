...
func _build_title() -> void:
	...
	# Generated by tools/branding/gen_wordmark.py; see INTEGRATION-branding.md.
	...
	# Spanning the FULL width and letting KEEP_ASPECT_CENTERED do the centring
	# is what actually centres it. PRESET_CENTER_TOP does not: Control.size
	# grows the right offset only, so the mark lands half its width off-centre.
	# Header budget, in the 1920x1080 UI space (NOT the 1280x720 window — the
	# project stretches canvas_items, so a screenshot is these numbers x0.667):
	# the ring's top crest starts at y = 1080*0.5 + 20 - 1080*0.36 - 118*0.5
	# = 112. So the header owns y 0..112 and the three lines are budgeted
	# 4..64 / 66..92 / 92..112 to land inside it with nothing behind a crest —
	# which the 34pt heading + subtitle did not manage before.
	mark.set_anchors_preset(Control.PRESET_TOP_WIDE)
	mark.offset_top = 4
	mark.offset_bottom = 64
	# Load-bearing: the crests are click/hover targets and this node is wide.
	# Without IGNORE it eats the ring's mouse events near the top of the screen.
